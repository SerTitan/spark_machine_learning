...
> ВНИМАНИЕ: этот файл писался в январе 2026 и частично устарел после рефакторинга 2026-04-24
> (перемещение старых WordCount-скриптов в `archive/legacy_scripts/`, планов и NIR в `docs/`,
> промежуточных CSV в `data/archive/`). Актуальная структура — в `README.md` и
> `docs/VKR_PLAN.md` §17. Файл оставлен как историческая справка по схеме датасета.
