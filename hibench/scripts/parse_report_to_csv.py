...
def parse_wc_runs_line(line: str):
    # ожидаем "wordcount, spark, tiny, 12.345"
...
def parse_report_line(line: str):
    # формат HiBench: "WordCount spark tiny 12.345 ..."
...
def emit_runs_csv(rows, meta):
    # Построчный CSV: один запуск = одна строка, + все параметры из meta
    fields = [
        ...
        "rdd_compress","io_compression_codec","input_gb","spark_conf"
    ]
...
def emit_agg_csv(rows, meta):
    # Агрегация по одной конфигурации (текущей): mean/median/p50/p90/p99/std/n
...
# 1) пробуем /tmp/wc_runs.txt (устойчивый путь из нашего раннера)
if runs_file.exists():
...
# 2) если wc_runs.txt пуст, попробуем hibench.report
