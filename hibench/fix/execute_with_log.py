...
def execute(workload_result_file, command_lines):
    ...
    for line in lines_iterator:
        count += 1
        if count > 100 or time()-last_time>1:
        ...
        def table_not_found_in_log(line):
            regex = re.compile(fnmatch.translate("*Table * not found*"))
            return bool(regex.match(line))

        def database_default_exist_in_log(line):
            return "Database default already exists" in line

        def uri_with_key_not_found_in_log(line):
            return "Could not find uri with key [dfs.encryption.key.provider.uri]" in line

        if ('error' in lline) and lline.lstrip() == lline:
            bypass_error_condition = table_not_found_in_log(line) or database_default_exist_in_log(lline) or uri_with_key_not_found_in_log(lline)
