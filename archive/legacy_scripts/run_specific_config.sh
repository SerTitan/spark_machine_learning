...
REPORT_JSON="${REPORT_JSON:-${PROJECT_DIR}/out/rl_topo336_constrained/report.json}"
...
_start_workers() {
  ...
  for i in $(seq 1 "$n"); do
    docker run -d --rm --name "spark-worker-$i" --hostname "spark-worker-$i" --network "${DOCKER_NET}" \
      ...
      -e SPARK_WORKER_CORES="${c}" -e SPARK_WORKER_MEMORY="${m}g" "${SPARK_WORKER_IMAGE}" >/dev/null
  done
  # Ждём готовности
  for s in {1..20}; do
