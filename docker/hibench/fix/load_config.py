...
def execute_cmd(cmdline, timeout):
    ...
    while p.poll() is None and (
        ...
        stdout += ((nonBlockRead(p.stdout) or b"" ).decode(errors="ignore"))
        stderr += ((nonBlockRead(p.stderr) or b"" ).decode(errors="ignore"))
        ...
        if not (HibenchConf.get("hibench.spark.master", "")):
            ...
            # spark standalone mode
                ...
                try:
                    ...
                    with closing(urllib.request.urlopen('http://%s:%s' % (HibenchConf['hibench.masters.hostnames'], master_port))) as page:
                        worker_hostnames = []
