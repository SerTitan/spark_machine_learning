...
#!/usr/bin/env python3
"""
train_dnn_qlearning_v2.py
- Same as v1, but trains on log1p(duration_s) to avoid negative predictions and stabilize loss.
- At inference and for metrics, predictions are transformed back via expm1 to seconds.
"""
...
RAW_FEATURES = [
    ...
    "memory_fraction",
    "shuffle_compress","spill_compress",
    "broadcast_block","maxSizeInFlight","shuffle_file_buffer",
    "io_codec",
    "duration_s","throughput_Bps","input_bytes","exit_code"
]
...
class TorchRegressor(nn.Module):
    def __init__(self, input_dim: int):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(4, 1)  # outputs log-duration
        )
    def forward(self, x):
        return self.net(x)
...
class TrainedModel:
    ...
    def save(self, out_dir: str):
        ...
        if self.use_torch:
            ...
            joblib.dump(self.model, os.path.join(out_dir, "model_sklearn.joblib"))
...
def train_torch(X_tr, y_tr_log, X_va, y_va, *, epochs=120, lr=1e-3, batch=64, seed=42, verbose=True):
    ...
    for ep in range(1, epochs+1):
        ...
        y_va_pred = np.expm1(y_va_log_pred)
        ...
        if val_rmse < best_val - 1e-6:
            ...
            if bad >= patience:
                if verbose:
                    print(f"[torch] early stop at epoch {ep}, best val_RMSE={best_val:.4f}")
                break
    if best_state:
        model.load_state_dict(best_state)
    return model, val_hist

def train_sklearn_mlp(X_tr, y_tr_log, X_va, y_va):
    from sklearn.neural_network import MLPRegressor
    mlp = MLPRegressor(hidden_layer_sizes=(12,8,4),
                       activation="relu",
                       solver="adam",
...
def plot_topk_bar(topk_profile_df: pd.DataFrame, median_val: float, out_path: str, title: str):
    ...
    vals = topk_profile_df["t_pred"].values
    idx = np.arange(len(vals)); labels = [str(i+1) for i in range(len(vals))]
...
class DiscreteQLearner:
    ...
    def _qrow(self, s: Tuple[int,...]):
        if s not in self.Q:
            self.Q[s] = {a:0.0 for a in self._neighbors(s)}
        return self.Q[s]
    ...
    def run(self, base_template: pd.DataFrame, max_episodes=50, max_steps=20, min_rel_improve=0.10):
        s_best = self._initial_state()
        conf_best = self._state_from_indices(s_best)
        ...
        for ep in range(max_episodes):
            ...
            for step in range(max_steps):
                ...
                if a is None: break
                s2 = self._apply(s,a)
                conf2 = self._state_from_indices(s2)
                ...
                s = s2; t_prev = t_new
                rel_impr = (t_best - t_prev)/t_best if t_best>0 else 0.0
...
def main():
    ...
    if use_torch:
        try:
            ...
            print(f"[WARN] Torch failed ({e}); falling back to sklearn."); use_torch=False
    if not use_torch:
        model, loss_curve = train_sklearn_mlp(Xtr, y_tr_log, Xva, y_va_sec)
        val_curve = list(loss_curve)
    ...
    tm = TrainedModel(pre=pre, model=model, use_torch=use_torch,
