...
while [[ "$samples_done" -lt "$TARGET_SAMPLES" ]]; do
  ...
  # Берём последние REPEATS строк из отчёта, извлекаем duration (столбец 5), считаем медиану
  median=$(tail -n "$REPEATS" "$REPORT" | awk '{print $5}' | sort -n | \
           awk '{a[NR]=$1} END{if(NR>0) print (NR%2) ? a[(NR+1)/2] : (a[NR/2]+a[NR/2+1])/2; else print "NA"}')
